 ...
 * Checks package.json dependencies against a registry of known vulnerable
 * npm packages and version ranges. No external API calls — runs offline
 * against a built-in advisory database.
 *
 * Uses a lightweight built-in semver comparator (no external dependency).
 */
...
// ---------------------------------------------------------------------------
// Advisory database — known vulnerable packages with affected version ranges.
// Each entry maps a package name to one or more advisories.
// ---------------------------------------------------------------------------
...
function cleanVersion(version: string): string | null {
  if (/^(workspace:|file:|git[+:]|https?:|ssh:|\*)/.test(version)) return null;
...
export function scanDependencies(packageJson: string): DependencyScanResult {
  ...
  try {
    ...
    return { ...emptyResult, scanned: false };
  }
  ...
  for (const [pkgName, versionSpec] of Object.entries(deps)) {
    const advisories = ADVISORY_DB[pkgName];
